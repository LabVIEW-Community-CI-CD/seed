...
Describe "Golden Sample Full Coverage — $SourceFile" {
    ...
    It "enumerates all aliases, generates a patch, and validates round-trip" {
        ...
        try {
            ...
            # Enumerate all field aliases (leaf paths)
            ...
            # Validate: only patched fields changed, all values match expected
            Compare-AfterPatch $orig $patched $patchMap
        }
        finally { Remove-Item $jsonOrig, $jsonPatched, $patchedFile, $patchYaml -ea SilentlyContinue }
    }
}
