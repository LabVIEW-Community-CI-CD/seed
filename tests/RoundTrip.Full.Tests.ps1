...
Describe "VIPB full serialization-fidelity round-trip" {
    BeforeAll {
        # Setup temporary file paths for conversion steps
        ...
        function Add-Paths ($obj, $prefix = "") {
            if ($obj -is [System.Collections.IDictionary]) {
                foreach ($k in $obj.Keys) { Add-Paths $obj[$k] ($prefix ? "$prefix.$k" : $k) }
            }
            elseif ($obj -is [System.Collections.IEnumerable] -and $obj -isnot [string]) {
                $i = 0; foreach ($v in $obj) { Add-Paths $v ($prefix ? "$prefix[$i]" : "[$i]"); $i++ } 
            }
            else {
                if ($prefix) { [void] $allPaths.Add($prefix) }
            }
        }
        Add-Paths $jsonObj ""
        ...
        function Get-ValueByPath($obj, [string[]] $parts, $index = 0) { 
            # (Recursive helper to get value at a given dotted path; omitted here for brevity) 
        }
        foreach ($path in $allPaths) {
            ...
            elseif($origVal -is [numeric]) { $newVal = try { [double]$origVal + 1 } catch { 1 } } 
            ...
            if ($origVal -ne $null -and $origVal -eq $newVal) {
                ...
                elseif($origVal -is [numeric]){ $newVal = $origVal + 1 }
                else                          { $newVal = "__PATCHED__" }
            }
        ...
        # Load original and patched JSON into objects for comparison in tests
