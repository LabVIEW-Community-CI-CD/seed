...
# run CLI
VipbJsonTool "$INPUT_MODE" "$INPUT_IN" "$INPUT_OUT" "$PATCH_FILE" /tmp/always_patch.yml "${INPUT_BRANCH_NAME:-}" "${INPUT_AUTO_OPEN_PR:-false}"

# bundle artifacts if requested
if [[ -n "${INPUT_UPLOAD_FILES:-}" ]]; then
  mkdir -p /tmp/vipb_art
  while IFS= read -r f; do cp "$f" /tmp/vipb_art/ || true ; done <<< "$INPUT_UPLOAD_FILES"
